...
export default class ProjectCards extends Component {
  ...
  getProjects() {
    ...
    for (
      let projectIndex = firstProject;
      projectIndex < lastProject;
      projectIndex++
    ) {
  ...
  getProjectIcons(project) {
    ...
    function getImgs(img, index) {
      icons.push(
        <li key={index}>
          <img className="project-icons" src={img} alt="tech-icon" />
        </li>
      );
    }
    ...
    if (firstProject === projectIndex) {
      return (
        <img
          ...
          alt="dotted-squares"
          className="card-deco-first"
          style={{ transform: 'translateY(' + this.state.offset * 0.1 + '%)' }}
        />
      );
      ...
      return (
        <img
          src="./images/decorations/dotted-square-blue.svg"
          alt="dotted-squares"
          className="card-deco-last"
          style={{
            transform: 'translateY(' + this.state.offset * 0.1 + '%)',
          }}
        />
      );
    }
